...
<head>
    ...
    <style>
        ...
        .hero_area {
            background-image: url('home/images/your-background-image.jpg');
            /* أضف رابط صورة الخلفية هنا */
        ...
        .product-image img {
            max-width: 100%;
            /* لضمان أن الصورة لا تتجاوز عرض الحاوية */
            max-height: 100%;
            /* تعيين الحد الأقصى للارتفاع للصورة */
            display: block;
            /* إزالة المسافة الزائدة أسفل الصور المدمجة */
            margin: 0 auto;
            /* تمركز الصورة أفقيًا */
        }
        ...
        .similar-products-heading {
            font-size: 28px;
            /* تعديل حجم الخط حسب الحاجة */
            color: #333;
            /* تغيير اللون حسب الرغبة */
            text-align: center;
            /* تمركز النص */
            margin-top: 20px;
            /* إضافة مسافة من الأعلى للتباعد */
            font-weight: bold;
            /* جعل النص عريضًا إذا رغبت */
        }
    </style>
</head>
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="product-details">
                        <form action="{{ url('product/' . $product->id . '/cart') }}" method="get"
                            ...
                            @endauth
                            <input type="text" name="url" value="{{ url()->previous() }}"
                                style="display: none;">
                            ...
                            <div class="custom-box"
                                ...
                                <b>
                                    <h3 class="text-primary">اسم المنتج: {{ $product->name }}</h3>
                                </b>
                            </div>

                            {{-- سعر المنتج --}}
                            <div class="custom-box"
                                style="background-color: #f5f5f5; padding: 10px; border-radius: 5px; margin-top: 10px;">
                                <h4 class="text-info">
                                    @if ($product->discounted_price != null)
                                        {{-- عرض السعر المخفض --}}
                                        <span class="original-price">السعر الأصلي: {{ $product->price }} ريال يمني</span><br>
                                        السعر المخفض:  {{ $product->discounted_price }} ريال يمني
                                    @else
                                        {{-- عرض السعر الأصلي --}}
                                        السعر: {{ $product->price }} ريال يمني
                                    @endif
                                </h4>
                            </div>
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <input type="number" name="quantity" value="1" min="1"
                                            value="{{ $product->quantity }}" style="width: 100px;" required>
                                    </div>
                                    <div class="col-md-6">
                                        <input type="submit" class="btn btn-danger" style="background-color: red;"
                                            value="أضف إلى السلة">
                                    </div>
                                </div>
                        ...
                        <a href="{{ route('product_details', ['id' => $relatedProduct->id]) }}">
                            <div class="product-image">
                                ...
                                <div class="product-hover">
                                    <a href="{{ route('product_details', ['id' => $relatedProduct->id]) }}"
                                        class="details-button">التفاصيل</a>
                                </div>
                            </div>
                            <a href="{{ route('product_details', ['id' => $relatedProduct->id]) }}"
                                class="product-title">
                            ...
                            <div class="product-price">
                                @if ($relatedProduct->discounted_price != null)
                                    <div class="original-price">
                                        السعر الأصلي:  {{ $relatedProduct->price }} ريال يمني
                                    </div>
                                    سعر العرض: {{ $relatedProduct->discounted_price }} ريال يمني
                                @else
                                    السعر:  {{ $relatedProduct->price }} ريال يمني
                                @endif
                            </div>
                        </a>
                    </div>
